ALTER TABLE books ADD COLUMN owner_user_id BIGINT NULL;
ALTER TABLE books ADD COLUMN access_scope VARCHAR(16) NOT NULL DEFAULT 'PUBLIC';
ALTER TABLE books ADD COLUMN origin_type VARCHAR(16) NOT NULL DEFAULT 'CRAWLER';
ALTER TABLE books ADD CONSTRAINT fk_books_owner FOREIGN KEY (owner_user_id) REFERENCES users (id) ON DELETE CASCADE;
CREATE INDEX idx_books_source_owner_url ON books (source_id, owner_user_id, source_url_hash);
ALTER TABLE books DROP CONSTRAINT uk_books_source_book;
ALTER TABLE books DROP CONSTRAINT uk_books_source_url_hash;
